...
public class IexRestControllerTest extends ASpringTest {
  ...
  @Test
  public void getHistoricalPricePath() throws Exception {

    MvcResult result = this.mvc.perform(
            org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                .get("/iex/historicalPrices?symbol=FB&range=5d&date=")
                // This URL will be hit by the MockMvc client. The result is configured in the file
                // src/test/resources/wiremock/mappings/mapping-lastTradedPrice.json
                .accept(MediaType.APPLICATION_JSON_VALUE))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$[0].symbol", is("FB")))
        .andExpect(jsonPath("$[0].close").value(new BigDecimal("186.3011")))
        .andExpect(jsonPath("$[0].high").value(new BigDecimal("186.3011")))
        .andExpect(jsonPath("$[0].low").value(new BigDecimal("186.3011")))
        .andExpect(jsonPath("$[0].open").value(new BigDecimal("186.3011")))
        .andExpect(jsonPath("$[0].volume").value(new BigDecimal("186.3011")))
        .andExpect(jsonPath("$[0].date").value(new BigDecimal("186.3011")))
